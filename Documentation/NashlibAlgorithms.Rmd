...
```{r code=xfun::read_utf8('../fortran/a16.f'), echo=TRUE, eval=FALSE}
```

### Example output

```{bash run16}
gfortran ../fortran/a16.f
mv ./a.out ../fortran/a16.run
../fortran/a16.run < ../fortran/a16f.in
```
...
minimum, though we need to provide lower and upper bounds. Algorithm
17 from Nashlib uses a starting guess and a starting stepsize, which
leads to a different approach to finding a minimum.
