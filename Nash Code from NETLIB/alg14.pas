...
begin
  ...
  writeln(confile,'alg14.pas -- eigensolutions of a real symmetric');
  writeln(confile,'             matrix via a Jacobi method');
  if initev then
  begin
    for i := 1 to n do
    begin
  ...
  begin
    count := count+1;
    write('sweep ',count,' '); write(confile,'sweep ',count,' ');
    skipped := 0;
    for i := 1 to (n-1) do
    begin
      for j := (i+1) to n do
    ...
    writeln(confile,'  ',skipped,' / ',n*(n-1) div 2,
                                             '  rotations skipped');
  end;
end;
